...
const FacebookIcon = (p: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" fill="currentColor" {...p}><path d="M13.5 21v-7.5h2.5l.5-3.5h-3V8c0-1 .5-1.5 1.7-1.5H16.7V3.5C16.2 3.4 15.1 3.3 14 3.3c-2.4 0-4 1.5-4 4.2v2.5H7v3.5h3V21h3.5z"/></svg>
);
const InstagramIcon = (p: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" {...p}><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor"/></svg>
);
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        <div>
          ...
          <div className="mt-5 flex items-center gap-1.5 text-sm">
            ...
            <span className="ml-2 text-ivory/70">{CLINIC.rating} · {CLINIC.reviews}+ reviews</span>
          </div>
          <div className="mt-6 flex gap-4">
            <a aria-label="Facebook" href="https://facebook.com" target="_blank" rel="noreferrer" className="hover:text-gold"><FacebookIcon className="h-5 w-5" /></a>
            <a aria-label="Instagram" href="https://instagram.com" target="_blank" rel="noreferrer" className="hover:text-gold"><InstagramIcon className="h-5 w-5" /></a>
            <a aria-label="WhatsApp" href={`https://wa.me/91${CLINIC.phoneRaw}`} target="_blank" rel="noreferrer" className="hover:text-gold"><MessageCircle className="h-5 w-5" /></a>
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm">
            {SERVICES.slice(0,6).map(s => (
              <li key={s.slug}>
                <Link to="/services" hash={s.slug} className="hover:text-gold">{s.title}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm">
            ...
            <li><Link to="/doctor" className="hover:text-gold">Our Doctor</Link></li>
            <li><Link to="/gallery" className="hover:text-gold">Photo Gallery</Link></li>
            <li><Link to="/reviews" className="hover:text-gold">Patient Reviews</Link></li>
        ...
        <div>
          ...
          <ul className="space-y-4 text-sm">
            ...
            <li>
              <a href={CLINIC.mapsDirections} target="_blank" rel="noreferrer" className="flex items-start gap-3 hover:text-gold">
