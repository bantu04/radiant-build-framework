...
export function BeforeAfter({ before, after, label }: { before: string; after: string; label?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 text-[10px] uppercase tracking-[0.2em] text-ivory bg-charcoal/70 backdrop-blur px-2 py-1 rounded-sm">Before</div>
      <div className="absolute top-4 right-4 text-[10px] uppercase tracking-[0.2em] text-charcoal bg-ivory/80 backdrop-blur px-2 py-1 rounded-sm">After</div>
      {label && <div className="absolute bottom-4 left-4 font-serif italic text-ivory text-lg drop-shadow">{label}</div>}
